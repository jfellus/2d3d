...
typedef struct {void(*f)(void* p, int); int first,nb; void* p;} _pthread_do_multithread_data;
typedef struct {void(*f)(void* p, int,int); int first,nb; void* p;} _pthread_do_multithread_range_data;

void* _pthread_do_multithread_range(void* d) {
	_pthread_do_multithread_range_data* data = (_pthread_do_multithread_range_data*)d;

	data->f(data->p, data->first,data->nb);
...
void do_multithread(void (*f)(void* p, int i), void* p, int n) {
	...
	for(int i=0; i<nbthreads; i++) {
		...
		d->f = f; d->first = first; d->nb = nb; d->p = p;
