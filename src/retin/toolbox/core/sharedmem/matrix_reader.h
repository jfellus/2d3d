...
public:
	matrix_reader(const char* filename, bool bFormatBinary = false) {
		...
		if(bFormatBinary) in.open(filename, std::ios::binary);
	...
	virtual void read (T*& buffer,size_t truncateDim=0, int max_height= -1) = 0;
...
public:
    ...
    virtual void        read (T*& buffer,size_t truncateDim=0, int max_height = -1);
...
public:
    ...
    virtual void read (float*& buffer,size_t truncateDim=0, int max_height = -1);
...
template<class T> csv_reader<T>::csv_reader(const char* filename) : matrix_reader<T>(filename, false),w(0),h(0) {
	char line[0x10000];
	char value[0x100];
...
template<class T> size_t csv_reader<T>::get_height() {return h;}
template<class T> size_t csv_reader<T>::get_width() {return w;}

template<class T> void csv_reader<T>::read (T*& data,size_t truncateDim, int max_height) {
	char line[0x10000];
	char value[0x100];
...
template<class T> txt_reader<T>::txt_reader(const char* filename) : matrix_reader<T>(filename, false),w(0),h(0) {
	...
	while (!this->in.eof()) {
		...
		this->in.getline(line,0x10000);
		if (line[0] == 0) break;
		if (this->in.fail()) throw "Input stream error 1";
		if (w == 0) {
			std::istringstream iss(line);
			while(iss.getline(value,0x100,' ')) w ++;
		}
		h ++;
	}
	this->in.clear();
...
template<class T> size_t txt_reader<T>::get_height() {return h;}
template<class T> size_t txt_reader<T>::get_width() {return w;}

template<class T> void txt_reader<T>::read (T*& data,size_t truncateDim, int max_height) {
	char line[0x10000];
	char value[0x100];
...
template<class T> general_fvec_reader<T>::general_fvec_reader(const char* filename)
...
template<class T> void general_fvec_reader<T>::read_feature (T* buffer,size_t truncateDim) {
    uint32_t dim = 0;
    this->in.read((char*)&dim,sizeof(dim));
    if (this->in.fail())
...
template<class T> void general_fvec_reader<T>::read (T*& buffer,size_t truncateDim, int max_height) {
	if(max_height==-1) max_height = get_height();
    if (!buffer) buffer = new T[ get_width()*(MIN(max_height,get_height()))];
    for (size_t i=0;i<MIN(max_height, h);i++)
...
template<class T> general_bin_reader<T>::general_bin_reader(const char* filename) : matrix_reader<T>(filename, true) {
	h=w=0;
	T x = 0;
...
template<class T> size_t general_bin_reader<T>::get_height() {return h;}
template<class T> size_t general_bin_reader<T>::get_width() {return w;}

template<class T> void general_bin_reader<T>::read (T*& data,size_t truncateDim, int max_height) {
	T x = 0;
